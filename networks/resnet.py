...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(torch.load("/home/zhao/saliency/1/pretrained_models/resnet/resnet50-19c8e357.pth"))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/zhao/saliency/1/pretrained_models/resnet/resnet101-5d3b4d8f.pth"))
            # remove fully connected layer, avg pool and layer5:
            #self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            self.resnet = resnet
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/zhao/saliency/1/pretrained_models/resnet/resnet152-b121ed2d.pth"))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
    ...
    def forward(self, x):
        ...
        output = self.layer5(x4)  # (shape: (batch_size, 4*512, h/16, w/16))
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/zhao/saliency/1/pretrained_models/resnet/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool and layer5:
            #self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            self.resnet = resnet
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load("/home/zhao/saliency/1/pretrained_models/resnet/resnet34-333f7ec4.pth"))
            # remove fully connected layer, avg pool and layer5:
            #self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            self.resnet = resnet
...
# model=ResNet101_OS16()
# input=torch.autograd.Variable(torch.randn(1,3,480,854))
# o=model(input)
# print(o.shape)